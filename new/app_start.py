...
def run_inference(face_model, image, input_type):
    all_known_face_encodings = load_face_encodings(face_model)

    if input_type == "Image":
        img = np.array(image)
        annotated_frame = process_frame_for_face_recognition(img, all_known_face_encodings, name_dict)
...
def app():
    with gr.Blocks():
        with gr.Row():
            with gr.Column():
                face_model = gr.Textbox(label="Face Model Path", value="C:\\Users\\U\\Desktop\\yolov10\\Chinese_license_plate_detection_recognition-main\\z\\facedata.pkl")
                input_type = gr.Radio(choices=["Image", "Video"], value="Image", label="Input Type")
                image_input = gr.Image(label="Input Image", visible=True)
                video_output = gr.Video(label="Output Video", visible=False)

                def update_visibility(input_type):
                    return gr.update(visible=(input_type == "Image")), gr.update(visible=(input_type == "Video"))

                input_type.change(fn=update_visibility, inputs=[input_type], outputs=[image_input, video_output])

                output_image = gr.Image(label="Annotated Image", visible=True)
                gr.Button(value="开始识别").click(fn=run_inference, inputs=[face_model, image_input, input_type], outputs=output_image)
